#!/usr/bin/env node
// Track A — clean, colourful, transparent pet sprites.
//
// Replaces the legacy imagetracer.js output (raster-traced PNGs full of
// near-white zero-opacity shards). Outputs compact procedural SVGs:
//   - viewBox 0 0 256 256
//   - transparent background
//   - no <text>, no <image>, no data:image
//   - under 20 KB per file (typically < 6 KB)
//
// Also performs the housekeeping the improvement plan calls for:
//   - deletes stale ASCII-art *.txt scratch files alongside each frame
//   - relocates *-reference.png raster refs to packages/core/assets/.sources/
...
const sourcesRoot = path.join(assetsRoot, ".sources");
...
const STATES = {
  healthy: { fg: "#4b3a2a", bg: "#fff8d2", accent: "#f3c95c", bubble: "#fff8d2" },
  sleep_deprived: { fg: "#3a3a66", bg: "#d5d8ff", accent: "#b9c1ff", bubble: "#d5d8ff" },
  stressed: { fg: "#a34a1f", bg: "#ffe1d4", accent: "#89a4d5", bubble: "#ffe1d4" },
  neglected: { fg: "#544e3c", bg: "#e6e2d4", accent: "#8e9b6e", bubble: "#e6e2d4" }
};
...
function framesForState(state) {
  if (state === "healthy") return [...BASE_FRAMES, "reaction-happy", "turn-3q", "turn-front"];
  if (state === "neglected") return [...BASE_FRAMES, "reaction-wilt"];
  return BASE_FRAMES;
}
...
function getPose(frame) {
  switch (frame) {
    ...
    case "turn-3q":
      return { y: 0, lean: 6, squash: 1, eyes: "open", talk: false, hop: 0 };
    case "turn-front":
...
function svgStyle(species, state, opacity) {
  ...
  return [
    ...
    `.belly-fill{fill:var(--color-species-${species}-belly,${palette.belly});opacity:${opacity};}`,
...
function faceMarkup(state, pose, leftX, rightX, eyeY, mouthCx, mouthY) {
  ...
  if (blink) {
    ...
    eyes = [
      ...
      `<ellipse fill="#ffffff" cx="${leftX + 2}" cy="${eyeY - 3}" rx="1.6" ry="2" opacity="0.9" />`,
      `<ellipse fill="#ffffff" cx="${rightX + 2}" cy="${eyeY - 3}" rx="1.6" ry="2" opacity="0.9" />`
    ].join("");
  }
...
function axolotlSvg(state, frame) {
  ...
  const y = pose.y;
  const bodyOpacity = state === "neglected" ? 0.86 : 1;
  const headY = 102 + y;
  ...
  return [
    ...
    `<ellipse class="body-fill outline-stroke" cx="128" cy="${headY - 8}" rx="54" ry="44" />`,
    `<ellipse class="belly-fill" cx="128" cy="${headY + 8}" rx="40" ry="24" opacity="0.78" />`,
    `<ellipse class="body-fill outline-stroke" cx="128" cy="${bodyY}" rx="48" ry="${Math.round(38 * pose.squash)}" />`,
    `<ellipse class="belly-fill" cx="128" cy="${bodyY + 6}" rx="30" ry="22" opacity="0.7" />`,
    ...
    `<circle class="blush-fill" cx="88" cy="${headY + 16}" r="9" opacity="${state === "neglected" ? "0.18" : "0.36"}" />`,
    `<circle class="blush-fill" cx="168" cy="${headY + 16}" r="9" opacity="${state === "neglected" ? "0.18" : "0.36"}" />`,
...
function catSvg(state, frame) {
  ...
  return [
    ...
    `<path class="blush-fill" d="M 96 ${headY - 32} L 86 ${headY - 52 - earLift} L 103 ${headY - 40} Z" opacity="0.75" />`,
    `<path class="blush-fill" d="M 160 ${headY - 32} L 170 ${headY - 52 - earLift} L 153 ${headY - 40} Z" opacity="0.75" />`,
    `<circle class="body-fill outline-stroke" cx="128" cy="${headY - 6}" r="48" />`,
    `<path class="belly-fill" d="M 100 ${headY} Q 128 ${headY + 26} 156 ${headY} Q 152 ${headY + 20} 128 ${headY + 26} Q 104 ${headY + 20} 100 ${headY} Z" opacity="0.72" />`,
    ...
    `<path class="belly-fill" d="M 114 ${bodyTop + 8} Q 128 ${bodyBottom - 12} 142 ${bodyTop + 8} Q 140 ${bodyBottom - 8} 128 ${bodyBottom - 6} Q 116 ${bodyBottom - 8} 114 ${bodyTop + 8} Z" opacity="0.7" />`,
    ...
    `<circle class="blush-fill" cx="96" cy="${headY + 16}" r="8" opacity="${state === "neglected" ? "0.14" : "0.28"}" />`,
    `<circle class="blush-fill" cx="160" cy="${headY + 16}" r="8" opacity="${state === "neglected" ? "0.14" : "0.28"}" />`,
    "</g></g>"
  ].join("");
}
...
function slimeSvg(state, frame) {
  ...
  return [
    ...
    `<g transform="translate(${lean} 0)">`,
    `<g transform="translate(0 ${y})">`,
    `<path class="body-fill outline-stroke" d="M 76 ${baseY - 10} Q 68 ${topY + 46} 90 ${topY + 10} Q 110 ${topY - 26 + domeLift} 128 ${topY - 28 + domeLift} Q 149 ${topY - 26 + domeLift} 168 ${topY + 10} Q 190 ${topY + 44} 180 ${baseY - 10} Q 170 ${baseY + drip} 150 ${baseY - 2} Q 136 ${baseY + 10} 128 ${baseY + 4} Q 117 ${baseY + 14} 102 ${baseY - 2} Q 84 ${baseY + drip} 76 ${baseY - 10} Z" />`,
    `<path class="glow-fill" d="M 106 ${topY - 4 + domeLift} Q 128 ${topY - 20 + domeLift} 150 ${topY - 4 + domeLift} Q 142 ${topY + 16 + domeLift} 128 ${topY + 14 + domeLift} Q 112 ${topY + 16 + domeLift} 106 ${topY - 4 + domeLift} Z" opacity="0.85" />`,
    ...
    `<circle class="blush-fill" cx="94" cy="${topY + 62}" r="8" opacity="${state === "neglected" ? "0.12" : "0.22"}" />`,
    `<circle class="blush-fill" cx="162" cy="${topY + 62}" r="8" opacity="${state === "neglected" ? "0.12" : "0.22"}" />`,
    frame === "reaction-happy"
...
function assertClean(svg, label) {
  if (/<text\b/i.test(svg)) throw new Error(`${label}: contains <text>`);
  if (/<image\b/i.test(svg)) throw new Error(`${label}: contains <image>`);
  if (/data:image/i.test(svg)) throw new Error(`${label}: contains data:image payload`);
  if (/imagetracer/i.test(svg)) throw new Error(`${label}: still looks like an imagetracer output`);
  const gz = gzipSync(Buffer.from(svg, "utf8")).length;
  if (gz > MAX_GZIP_BYTES) {
    throw new Error(`${label}: ${gz} bytes gzipped exceeds ${MAX_GZIP_BYTES} budget`);
  }
  return gz;
}

async function sweepStaleFiles() {
  const moved = [];
  const removed = [];
  const speciesDirs = await readdir(petsRoot, { withFileTypes: true });
  for (const sp of speciesDirs) {
    if (!sp.isDirectory()) continue;
    const spPath = path.join(petsRoot, sp.name);
    const states = await readdir(spPath, { withFileTypes: true });
    for (const st of states) {
      if (!st.isDirectory()) continue;
      const stPath = path.join(spPath, st.name);
      const files = await readdir(stPath);
      for (const f of files) {
        const full = path.join(stPath, f);
        if (f.endsWith(".txt")) {
          await rm(full);
          removed.push(full);
        } else if (f.includes("-reference") && f.endsWith(".png")) {
          const dest = path.join(sourcesRoot, "pets", sp.name, st.name, f);
          await mkdir(path.dirname(dest), { recursive: true });
          await rename(full, dest);
          moved.push(`${full} -> ${dest}`);
        }
      }
    }
  }
...
async function main() {
  ...
  for (const species of Object.keys(SPECIES)) {
    ...
    for (const state of Object.keys(STATES)) {
      ...
      for (const frame of framesForState(state)) {
        ...
        const label = `${species}/${state}/${frame}`;
        const gz = assertClean(svg, label);
        if (gz > maxGz) {
          maxGz = gz;
          maxGzLabel = label;
        }
  ...
  await writeFile(path.join(generatedRoot, "pet-svg.ts"), buildModule(spriteMap), "utf8");

  const { moved, removed } = await sweepStaleFiles();

  console.log(`generated sprites for ${Object.keys(SPECIES).length} species x ${Object.keys(STATES).length} states`);
  console.log(`max gzipped size: ${maxGz} bytes (${maxGzLabel}) — budget ${MAX_GZIP_BYTES}`);
  if (removed.length) console.log(`removed ${removed.length} stale .txt scratch files`);
  if (moved.length) console.log(`moved ${moved.length} raster references -> ${path.relative(repoRoot, sourcesRoot)}/`);
}
